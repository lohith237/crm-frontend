...
export default function FormParser({
    ...
    if (modelObject?.orderBy && Array.isArray(modelObject.orderBy)) {
        modelObject.orderBy.forEach((order_obj) => {
            (modelObject.fields || []).forEach((field_obj) => {
                if (field_obj.fieldName === order_obj) {
                    ...
                    if (flagAddClass && modelObject.layoutClass) {
                        field_obj.groupClass = modelObject.layoutClass;
                    }
                    if (field_obj.fieldType === "group" && Array.isArray(field_obj.fields)) {
                        field_obj.fields.forEach((nestedField) => {
                            nestedField.groupClass = "col-md-6";
                        });
                    }
        ...
        (modelObject.fields || []).forEach((field_obj) => field_objects.push(field_obj));
    }
    ...
    const validateAll = () => {
        ...
        for (const fo of field_objects) {
            if (fo.fieldType !== "string") continue;
